...
class Base_model():
    ...
    def get_sigma_values(self):
        # 返回sigma1和sigma2的实际取值
        return self.net.sigma1.item(), self.net.sigma2.item()

    # def get_ln_values(self):
    #     return torch.abs(torch.log(self.net.sigma1.clone() * self.net.sigma2.clone()))
    ...
    def setup_optimizer(self):
        ...
        optim_args = self.opt['model']['optim']
        if optim_type == 'AdamW':
            params = list(self.net.parameters()) + [self.net.sigma1, self.net.sigma2]
            self.optim = torch.optim.AdamW(params, lr=self.init_lr, **optim_args)
        elif optim_type == 'SGD':
            params = list(self.net.parameters()) + [self.net.sigma1, self.net.sigma2]
            self.optim = torch.optim.SGD(params, lr=self.init_lr, **optim_args)
        elif optim_type == 'Adam':
            params = list(self.net.parameters()) + [self.net.sigma1, self.net.sigma2]
            self.optim = torch.optim.Adam(params, lr=self.init_lr, **optim_args)
        else:
    ...
    def setup_loss(self):
        ...
        for loss in loss_opt:
            ...
            if loss_opt[loss]['type'] == 'Baseloss':
            #if loss_opt[loss]['type'] == 'Bce_loss':boundary  TverskyLoss   Baseloss  Bce_loss
                self.loss_weight[loss_opt[loss]['type']] = self.net.sigma1
                #self.loss_weight[loss_opt[loss]['type']] = torch.div_(1, torch.mul_(2, torch.square(self.net.sigma1)))
                #self.loss_weight[loss_opt[loss]['type']] = 1 / (2 * torch.square(self.net.sigma1.clone()))
                # 通过在计算self.loss_weight[loss_opt[loss]['type']]时使用.clone()来避免原地操作，不会影响可学习参数self.net.sigma1的梯度计算和更新。
                # self.net.sigma1仍然会根据计算图正确地进行梯度反向传播和参数更新

                #self.loss_weight[loss_opt[loss]['type']] = (1 / (2 * torch.square(self.net.sigma1.clone()))) * (2*self.net.sigma2.clone() / (self.net.sigma2.clone() + self.net.sigma1.clone()))
            elif loss_opt[loss]['type'] == 'TverskyLoss':
                # SoftDiceLoss,Dice_loss,Iou_loss,SoftIouLoss  TverskyLoss
                self.loss_weight[loss_opt[loss]['type']] = self.net.sigma2
                #self.loss_weight[loss_opt[loss]['type']] = torch.div_(1, torch.mul_(2, torch.square(self.net.sigma2)))
                #self.loss_weight[loss_opt[loss]['type']] = 1 / (2 * torch.square(self.net.sigma2.clone()))

                #self.loss_weight[loss_opt[loss]['type']] = (1 / (2 * torch.square(self.net.sigma2.clone()))) * (2*self.net.sigma1.clone() / (self.net.sigma2.clone() + self.net.sigma1.clone()))
    ...
    #         out_dict[k] = v.item()
    #     return out_dict
    def dict_wrapper(self, input_dict):
        out_dict = {}
        for k, v in input_dict.items():
            if isinstance(v, torch.Tensor):
                out_dict[k] = v.item()
            else:
                out_dict[k] = torch.tensor(v).item()
        return out_dict

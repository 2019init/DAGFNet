...
class Test_model(Base_model):
    ...
    def test_one_iter(self, data):
        with torch.no_grad():
            ...
            #pred = self.net(img)
            #pred,pred1,pred2,pred3,pred4 = self.net(img)
            pred = self.net(img)
            ...
            #pred1 = F.interpolate(pred1, mask.shape[2:], mode='bilinear', align_corners=False)
            #pred2 = F.interpolate(pred2, mask.shape[2:], mode='bilinear', align_corners=False)
            #pred3 = F.interpolate(pred3, mask.shape[2:], mode='bilinear', align_corners=False)
            #pred4 = F.interpolate(pred4, mask.shape[2:], mode='bilinear', align_corners=False)
